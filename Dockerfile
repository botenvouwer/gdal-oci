...
# Oracle OCI arguments
ARG ORACLE_VERSION=19.8
ARG ORACLE_HOME=/usr/lib/oracle/${ORACLE_VERSION}/client64
...
#Setup build env for Oracele OCI
RUN apt-get -y update && \
  apt-get install -y wget gnupg2 && \
  apt-get -y install alien libaio1

# Install Oracle client (OCI)
...
# Add Oracle OCI
RUN apt-get -y install libaio1 gnupg2

ARG ORACLE_VERSION
ARG ORACLE_HOME
ENV ORACLE_HOME=$ORACLE_HOME
COPY --from=builder  $ORACLE_HOME $ORACLE_HOME
COPY --from=builder  /usr/include/oracle/${ORACLE_VERSION}/client64/ /usr/include/oracle/${ORACLE_VERSION}/client64/
